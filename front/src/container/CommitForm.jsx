...
function Tag(props) {
	return (
		<a
			...
			className={cn(
				"w-full text-center border-2 py-2 text-xl rounded-lg",
				props.selected
...
function evaluateOtherDay(state) {
	...
	for (let i = 0; i < 6; i += 2) {
		a =
			a &&
			state[keys[sanitise(i)]] &&
			!state[keys[sanitise(i - 1)]] &&
			!state[keys[sanitise(i + 1)]];
	}
...
export default function CommitForm(props) {
	...
	const reducer = (prev, action) => {
		...
		switch (action.type) {
			case "SET_EVERYDAY":
				let prevState = Object.keys(prev).reduce(function (
					previous,
					key
				) {
		...
		<section className="w-full absolute inset-0 z-[999] h-full flex flex-col items-center pt-12">
			...
			<form
				...
				onSubmit={(e) => {
					...
					while (true) {
						if (
							props.data.tasks.filter((i) => i.id === id).length <
							1
						) {
							break;
						}
					...
					if (days.length < 1 && title.length < 1) {
						return;
					}
					props.dispatch({
						type: "ADD_A_TASK",
			...
			>
				...
				<label className="w-full mb-16">
					...
					<div className="flex items-center justify-between mb-3 gap-3">
						<Tag
							text="everyday"
							selected={Object.keys(state).reduce(function (
								previous,
								key
							) {
						...
						<Tag
							...
							selected={evaluateOtherDay(state)}
							onClick={() => {
								dispatch({
