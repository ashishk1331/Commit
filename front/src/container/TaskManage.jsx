				...
				{props.data.tasks.length > 0 ? (
					props.data.tasks.map((i) => (
						<li key={i.id} className="w-full h-flex">
							<div
								className={twMerge(
									"w-full flex justify-between my-4 border-2 border-secondary-100 rounded-xl overflow-hidden p-5",
									editingMode && "flex-col items-left"
								)}
							>
								<h1
									className={twMerge(
										...
										edited.includes(i.id)
											? "line-through text-secondary-100"
											: ""
									)}
								>
									{i.title}
					...
					<>
						<p className="text-secondary-100 text-2xl text-center my-4">
